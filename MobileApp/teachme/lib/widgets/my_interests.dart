...
class _MyInterestsState extends State<MyInterests> {
  ...
  GestureDetector _accountButton(BuildContext context) {
    return GestureDetector(
      onTap: () async {
        ...
        if (result == true) await _reloadUserFromBackend();
  ...
  GestureDetector _payedButton(BuildContext context) {
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          PageRouteBuilder(
            pageBuilder:
                (_, __, ___) => PayedCoursesPage(), // Tu pantalla destino
            transitionsBuilder: (_, animation, __, child) {
              const begin = Offset(1.0, 0.0); // De derecha a izquierda
              const end = Offset.zero;
              const curve = Curves.ease;
      ...
      child: Container(
        ...
        child: _buttonRow('Cursos comprados', Icons.shopping_bag_outlined),
      ),
    );
  }
